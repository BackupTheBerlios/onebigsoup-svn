...
#
#  NOTE:
#    NOT written by the One Big Soup project.
#    This is just some code we picked up
#    from:   http://err.no/pymarkdown/
#    I believe it is written
#    by Tollef Fog Heen tfheen@raw.no
...
def _DoCodeSpans(text):
    ...
    (?!`)
    """
    pat = re.compile(match, re.I + re.VERBOSE)
...
def _DoItalicsAndBold(text):
    ...
    pat1 = re.compile(r"(\*\*|__) (?=\S) (.+?) (?<=\S) \1", re.VERBOSE + re.S)
