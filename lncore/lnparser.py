...
Parses v1.1 Local Names namespace description files.

LocalNamesSyntaxError  -- exception representing parse error
escape  -- escape column entry
unescape  -- unescape column entry
parse  -- parse namespace description
"""
import re
import urllib
...
_column_re_str = r'\s+("(?:(:?(?![\\]")[^"]|[\\]")+)"|(?:[^\s]+))'
column_re = re.compile(_column_re_str, re.UNICODE)
...
class LocalNamesSyntaxError(Exception):
    ...
    def __init__(self, msg):
        self.msg=msg
        err_str = "Local Names v1.1 Syntax Error: " + msg
        Exception.__init__(self, err_str)


def escape(text):
    r"""Escape text for a namespace token.

    Return a token for use in a namespace description.
    """
    text = text.replace("\\", "\\\\")
    text = text.replace('"', '\\"')
    return text
...
def unescape(token):
    r"""Escape text from a namespace token.

    Return text of token found in a namespace description.
    """
    text = text.replace('\\"', '"')
    text = text.replace('\\\\', '\\')
    return text
...
def parse_text(text):
    ...
    """Parse Local Names v1.1 description, return results and errors.

    Input text must be UTF-8.

    The function returns a tuple with the following form:

    (results, errors)

    Results have the form: (line#, "record type",
    ...
    def parse_record_type(cursor, line, parsed):
        ...
        if mo == None:
            err_str = ("unrecognized v1.1 record type, " +
                       "require LN, NS, X, or PATTERN")
            raise LocalNamesSyntaxError(err_str)
    ...
    def parse_column(cursor, line, parsed, col_num):
        ...
        if mo == None:
            err_str = "poorly formatted column #%d" % col_num
            raise LocalNamesSyntaxError(err_str)
        text=mo.group(1)
        if text[0] == u'"':
            text = text[1:-1]
